...
const Hatching = () => {
  ...
  const hatch = async () => {
    try {
      ...
      if (hatchPrice == 0) {
        const minterClient = minterContract.use(MINTER_CONTRACT_ADDRESS)
        await fetchWithToast(minterClient.hatchEggFree(wallet.address, selectedItem))
      } else {
        const client = cw20DRGN(wallet.getClient())
        await fetchWithToast(client.send(wallet.address, hatchPrice, selectedItem))
      }
  ...
  if (error) {
    ...
    return (
      <Grid container justifyContent="center" alignItems="center" sx={{ height: '100%', zIndex: 100 }}>
