...
const Inventory = () => {
  ...
  const [selectedIsStaked, setSelectedIsStaked] = useState(false)
  ...
  const [isDragonPage, setIsDragonPage] = useState(inventoryType === 'dragon')
  ...
  const loadMore = async () => {
    try {
      ...
      if (!isDragonPage) {
        ...
        let newDragonStartAfter =
          ownedNewDragonList.length > 0
            ? ownedNewDragonList[ownedNewDragonList.length - 1].token_id
            : null
        if (hasStarter) {
        ...
        const new_dragon_client = dragonContract?.use(UPDATED_DRAGON_CONTRACT_ADDRESS)
        ...
        const res_new_dragon = await new_dragon_client.rangeUserDragons(
          wallet.address,
          newDragonStartAfter,
          dragonLimit,
        )
        let all_items = ownedDragonList.concat(res_dragon)
        let all_new_items = ownedNewDragonList.concat(
          res_new_dragon.map((newDragonInfo) => ({ ...newDragonInfo, isNewDragon: true })),
        )
        ...
        await setOwnedNewDragonList(all_new_items)
        ...
        setHasNext(
          res_dragon.length >= dragonLimit ||
            ...
            res_new_dragon.length >= dragonLimit,
        )
        setIsFetching(false)
      }
    } catch (e) {
  ...
  useEffect(() => {
    const queryUserEggs = async () => {
      try {
        ...
        setIsFetching(false)
        setError(listedEggs.length === 0 && ownedEggList === 0)
      } catch (e) {
        setIsFetching(false)
      }
    }
  ...
  useEffect(() => {
    setIsDragonPage(inventoryType === 'dragon')
  }, [inventoryType])

  const scrollToSelected = async (item, isEgg, id, price, staked) => {
    await setSelected(true)
    await setIsEgg(isEgg)
    await setSelectedItem(item)
    await setSelectedItemId(id)
    await setSelectedItemPrice(price)
    await setSelectedIsStaked(staked ? staked : false)
  }
  ...
  const transferOnClick = async () => {
    try {
      ...
      const client = isEgg
        ...
        : dragonContract.use(
            selectedItem.isNewDragon ? UPDATED_DRAGON_CONTRACT_ADDRESS : DRAGON_CONTRACT_ADDRESS,
          )
    ...
    if (selectedItem && selectedItem.kind === 'starter') {
      ...
      return (
        <Grid item xs={12} lg={4}>
          ...
          {!isEgg && <DragonInfo inventory={true} item={selectedItem} staked={selectedIsStaked} />}
          ...
          <Grid>
            ...
            >
              ...
              <Grid container direction={'row'} marginTop={5} height="auto" overflow="auto">
                {isDragonPage &&
                listedDragons.length === 0 &&
                ownedDragonList.length === 0 &&
                ownedNewDragonList.length === 0 ? (
                  ...
                  <>
                    {isDragonPage &&
                      [...ownedDragonList, ...ownedNewDragonList].map((item, idx) => {
                        return (
                          <Grid key={idx} m={1} item sx={{ zIndex: 100 }}>
                            {item.kind !== 'starter' ? (
                              <Grid>
                                <InventoryDragon
                                  height={'410px'}
                ...
                {isDragonPage &&
                  listedDragons.map((item, idx) => {
                    return (
                      <Grid key={idx} m={1} sx={{ zIndex: 100 }}>
                        <InventoryDragon
                          height={'410px'}
                          maxWidth={'275px'}
                          imgHeight={'180px'}
                ...
                {!isDragonPage && ownedEggList.length === 0 && listedEggs.length === 0 ? (
