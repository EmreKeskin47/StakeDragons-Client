...
const Dragondrop = () => {
  ...
  const claimDragon = async () => {
    try {
      ...
      if (!isMember) {
        showFailToast('Claim Failed', 'Not eligible for dragondrop')
      } else {
        let res = await client.getTokens(wallet.address)
        if (res && res.length !== 0) {
          showFailToast('Claim Failed ', 'Already Claimed')
        } else {
          await fetchWithToast(minterClient.dragonDrop(wallet.address))
        }
        await whitelistQuery()
      }
    } catch (err) {
      showFailToast('Transaction Failed ', 'Click here for TX')
    }
  }
